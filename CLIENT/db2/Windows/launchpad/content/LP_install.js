function displayinstalled() {
  ...
  for (i=0; i<numvars; i++) {
    if (top.instArray[i][0] != undefined) {
      ...
      //Get the on screen elements for this product
      var prodElem = document.getElementById("prod_"+prod);
      var btnNew   = document.getElementById("btn_"+prod+"_new"); 
      var btnExist = document.getElementById("btn_"+prod+"_exist");
      var btnMigr  = document.getElementById("btn_"+prod+"_migr");
      var btnPrereqError = document.getElementById("btn_"+prod+"_error");
 
      if ((isAvail == "1") && (prodElem != null)) {
        prodElem.style.display="block";        
        ...
        switch (isInst) {
            ...
            case "1":       //New and Existing
            {
                btnNew.style.display = "inline";
                btnExist.style.display = "inline";
                break;
            }    
            case "2":       //New and Migrate --> Migrate is replaced by Work with Existing. Migrate will be handled in the picker.
            {
                btnNew.style.display = "inline";
                btnExist.style.display = "inline";
                break;
            }    
            case "3":       //New, Existing and Migrate --> Migrate is replaced by Work with Existing. Migrate will be handled in the picker.
            {
                btnNew.style.display = "inline";
                btnExist.style.display = "inline";
                break;
            }    
            case "4":       // 32bit DB2 is found in the system, 64bit DB2 is installing. 
            {
                btnExist.style.display = "inline";
                break;
            }    
            case "5":       //Existing only
            {
                btnExist.style.display = "inline";
                break;
            }    
            case "6":       // missing DB2 product prerequisite. This is for the add-on products.
            {
                btnPrereqError.style.display = "inline";
            }
            default:
            {
            	  break;
...
function installNew(e) {
    if (isIE()) {
        var cmd = "-e " + e.srcElement.name + " " + top.commandArgs;
    } else {
        var cmd = "-e " + e.target.getAttribute("name") + " " + top.commandArgs;
    }
    var fullcmd = top.baseDir + "db2\\Windows\\wininst.exe " + cmd;
    WriteCmdLineToFile(fullcmd);
}

function installExisting(e){
    if (isIE()) {
        if (e.srcElement.name == "doce") {
            installNew(e);
            return;
        }
        else {
            var cmd = "-lp " + top.langArg + " -e " + e.srcElement.name + " " + top.commandArgs;
        }
    } else {
        if (e.target.getAttribute("name") == "doce") {
            installNew(e);
            return;
        }
        else {
            var cmd = "-lp " + top.langArg + " -e " + e.target.getAttribute("name") + " " + top.commandArgs;
        }
    }
    var fullcmd = top.baseDir + "db2\\Windows\\DB2InstSel.exe " + cmd;
    WriteCmdLineToFile(fullcmd);
}

function migrate(e){
    if (isIE()) {
        var cmd = "-migrate -e " + e.srcElement.name + " " + top.commandArgs;
    } else {
        var cmd = "-migrate -e " + e.target.getAttribute("name") + " " + top.commandArgs;
    } 
    var fullcmd = top.baseDir + "db2\\Windows\\wininst.exe " + cmd;
    WriteCmdLineToFile(fullcmd);    
}    
...
function execute(command, commandParam)
{
  ...
  if (isIE()) {
    ...
    try {
      ...
      commandArray = commandParam.split(" ");
      
      process.init(program); 
      process.run(false, commandArray, commandArray.length - 1, {});
      exit();
    } catch (e) {
      alert("exception " + e.name + ': ' + e.message);
    }
...
function WriteCmdLineToFile(sText){

    if (isIE()) {
        var fso, fileObj, path;
        
        fso = new ActiveXObject("Scripting.FileSystemObject");
        path = fso.GetSpecialFolder(2);
        path += "\\DB2InstCmdLine.tmp";
       
        fileObj = fso.OpenTextFile(path,2,true,0);
        
        // write the whole command line into temp file
        // db2lp.exe read command from it and then create process
        fileObj.write(sText)
        fileObj.close()
        top.Exit();
    } else {
...
// SIG // 7wam66Fp+7uFPCsObfF6UevsnMlpswUa4SAUiW61n8eg
...
// SIG // BgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiB